...
def Search(request):
    ...
    if 'q' in request.GET:
        ...
        object_list = Customer.objects.filter(
            first_name__startswith=q)  # filter through the objects and see if a name matches 'q'
...
def Search_Product(request):
    ...
    if 'q' in request.GET:  #
        ...
        object_list = Product.objects.filter(
            title__startswith=q)  # filter through the objects and see if a name matches 'q'
        if (len(object_list) <= 0):
